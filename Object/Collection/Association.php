...
namespace CRUDsader\Object\Collection {
        class Association extends \CRUDsader\Object\Collection implements \SplObserver {
                ...
                public function getForm($oql = false, $alias = false, \CRUDsader\Form $form = null) {
                        ...
                        $max = $this->_definition['reference'] == 'internal' ? 1 : ($this->_definition['max'] == '*' ? \CRUDsader\Instancer::getInstance()->configuration->map->defaults->associations->compositionObjectDisplayInForm : $this->_definition['max']);
                ...
                public function update(\SplSubject $component) {
                        if ($component instanceof \CRUDsader\Form\Component && $component->hasParameter('compositionIndex')) {
                                ...
                                $value = $this->_getIdFromComponentValue($component);
                                ...
                                if ($target->isPersisted()) {
                                        ...
                                        if ($value != $target->isPersisted()) {// replace by proxy
                                                ...
                                                $this->_objects[$index] = $this->_replaceObjectByProxy($component);
                                                $this->_isModified = true;
                                                \CRUDsader\Object\Writer::setModified($this->_linkedObject);
                                        }else{
                                                // nothing has changed : same value than before
                                        }
                ...
                protected function _getIdFromComponentValue($component){
                        return $component->getValue();
                }

                protected function _replaceObjectByProxy($component) {
                        return \CRUDsader\Instancer::getInstance()->{'object.proxy'}($this->_class, $component->getValue());
                }
