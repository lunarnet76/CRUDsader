			...
			foreach ($rows as $i => $row) {
				...
				if (isset($id)) {
					if (!isset($collection->_objectIndexes[$id])) {
						if (\CRUDsader\Object\IdentityMap::exists($collection->_class, $id)) {
							$collection->_objects[$collection->_iterator] = \CRUDsader\Object\IdentityMap::get($collection->_class, $id);
						} else {
