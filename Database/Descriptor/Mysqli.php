        ...
        class Mysqli extends \CRUDsader\Database\Descriptor {
                ...
                public function createTable($name, array $fields, array $identity = array(), array $surrogateKey = array(), array $indexes = array()) {
                        ...
                        foreach ($fields as $name => $field) {
                                ...
                                $sql.=$this->quoteIdentifier($name) . ' ' . $field['type'] . (!empty($field['length']) ? '(' . $field['length'] . ')' : '') . (isset($field['charset']) ? ' COLLATE ' . $field['charset'] . ' ' : '') . (isset($field['null']) && $field['null'] ? ' NULL' : ' NOT NULL') . ',';
                        }
                        $sql[strlen($sql) - 1] = ' ';
                        if (!empty($surrogateKey)) {
                        ...
                        foreach ($indexes as $name => $keys) {
                                ...
                                $tmp = array();
                                foreach ($keys as $key){
                                        if(empty($surrogateKey) || ($surrogateKey['name']!=$key && count($keys) == 1))
                                                $tmp[] = $this->quoteIdentifier($key);
                                }
                                if(count($tmp))
                                        $sql.= ',KEY ' . $this->quoteIdentifier($name) . ' ('.implode(',', $tmp) . ')';
                        }
