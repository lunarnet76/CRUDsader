...
namespace CRUDsader {
        ...
        class Object implements Interfaces\Initialisable, Interfaces\Arrayable, \SplObserver, \SplSubject {
                ...
                public function copyFromObject(\CRUDsader\Object $object) {
                        foreach ($object->_fields as $k => $v) {
                                if ($this->hasAttribute($k))
                                        $this->getAttribute($k)->setValueFromInput($v->getValue());
                        }

                        foreach ($object->_associations as $k => $v) {
                                foreach ($v as $o) {
                                        $myO = \CRUDsader\Instancer::getInstance()->object($v->getClass());
                                        $definition = $v->getDefinition();
                                        if ($definition['composition']) {
                                                $myO->copyFromObject($o);
                                                $this->{$k}[] = $myO;
                                        } else {
                                                $this->{$k}[] = $o->getId();
                                        }
                ...
                public function castToParent(\CRUDsader\Object $object) {
                        $parent = $this->getParent();
                        foreach ($object->_fields as $k => $v) {

                                if ($parent->hasAttribute($k))
                                        $parent->getAttribute($k)->setValueFromInput($v->getValue());
                        }
                        foreach ($object->_associations as $k => $v) {
                                foreach ($v as $o) {
                                        $myO = \CRUDsader\Instancer::getInstance()->object($v->getClass());
                ...
                public function receiveArray(array $array, $evenIdField = false) {
                ...
                public function forceIsModified($bool) {
                ...
                public function getAttribute($name) {
                        if (!isset($this->_fields[$name]) && isset($this->_infos['attributes'][$name]) && isset($this->_infos['attributes'][$name]['extra']) && $this->_infos['attributes'][$name]['extra']) {
                                $type = strpos($name, '_id') !== false ? $this->_map->classGetFieldAttributeIdType($this->_class) : $this->_map->classGetFieldAttributeDefaultType();
