...
class Art_Debug {
    ...
    public static function setConfiguration(Art_Block $configuration){
    ...
    public static function chrono_start($index='script',$microtime=true){
        if(!self::$_configuration->php)return;
        if($microtime)
    ...
    public static function pre($v,$title='',$color='#E9E9E9') {
        ...
        if (!is_array($v) && !is_object($v)) {
           ...
           // $content=ob_get_clean();
            ...
            if(strlen($content)>100000)return substr($content,0,100000);
            $content = preg_replace_callback('/\[([^\]]*)\]/',array('Art_Debug','preCallback'),$content);
