...
class Art_Mvc_Link {
    ...
    public function __toString() {
        ...
        if (isset($this->_image))
            return '<a '.(isset($this->_id)?'id="'.$this->_id.'"':'').' href="' . $url . '"  ' . ($this->_css ? 'class="' . $this->_css . '"' : '') . ' ' . ($this->_target ? 'target="' . $this->_target . '"' : '') . '>' . ($this->_table ? '<table class="link"><tr><td class="image">' . $this->_image . '</td><td class="text">' . ($this->_text ? $this->_text : '') . '</td></tr></table>' : $this->_image . $this->_text) . '</a>';
        else
