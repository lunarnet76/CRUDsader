...
namespace CRUDsader\Map\Loader {
	...
	class Xml extends \CRUDsader\Map\Loader {
		...
		public function __construct()
		{
			...
			$this->_cacheManager = \CRUDsader\Instancer::getInstance()->cache->void;

			$this->_cacheIndex = 'map_loader' . md5($this->_file);

			if (false === $data = $this->_cacheManager->get($this->_cacheIndex)) {
		...
		public function getSchema(\CRUDsader\Block $defaults=null)
		{
			if (!$this->_cache) {
				...
				$this->_cacheManager->set($this->_cacheIndex,$ret);
			}else {
				$ret = $this->_cacheManager->get($this->_cacheIndex);
			}
			return $ret;
		}
	}
